...
function getPanelForView(panelview) {
    let node = panelview;
...
function show(panelview, button, window) {
    ...
    if(MainMenu.contains(button, window)) {
        ...
        buttonView = CustomizableUI.getWidget(buttonId).forWindow(window).anchor;
    }
...
function isShowing(panelview) {
    // The first condition checks if the panel of the panelview is open
    // The second condition checks if the panelview is opened as subview
    // if it isn't, it could be opened as mainView in a standalone
    // panel.
    return getPanelForView(panelview).state != "open" && (!!(panelview.getAttribute("current") || ( panelview.panelMultiView && panelview.panelMultiView.getAttribute("viewtype") == "main" && !panelview.panelMultiView.showingSubView )));
};
...
function hide(panelview, window) {
    if(isShowing(panelview)) {
        ...
        if(panelview.getAttribute("current")) {
            ...
            getPanelForView(panelview).hidePopup();
        }
    }
}
exports.hide = hide;
...
function itemClickHidesPanel(item) {
    return !item.hasOwnProperty("actionType") || (item.actionType != "radio" && item.actionType != "checkbox");
}
    ...
    actions.forEach(function(item) {
        ...
        if(item.type == "button") {
            ...
            if(!itemClickHidesPanel(item))
                ...
                node.setAttribute("closemenu", "single");
