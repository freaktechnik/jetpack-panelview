...
const { pipe, stripListeners } = require("sdk/event/utils");
...
const { WorkerHost, getAttachEventType } = require("sdk/content/utils");
...
function workerFor(panelview) workers.get(panelview);
...
const PanelView = Class({
    ...
    setup: function(options) {
        ...
        if(!options.contentURL)
            ...
            panelviewElement.addFrame(element, options.contentURL);
        }
        ...
        // setup worker
        if(options.contentURL) {
            let worker = new Worker(stripListener(options));
            workers.set(this, worker);
            pipe(worker, this);
            panelviewElement.listenFor(element, getAttachEventType(options), function(event) {
                worker.attach(panelviewElement.getContentDocument(element).ownerWindow);
            });
        }
